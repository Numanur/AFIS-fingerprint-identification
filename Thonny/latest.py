...
# detect: asks person_id, captures index & thumb (2 images each), saves as {pid}_{finger}_{n}.png
...
HOST, PORT, PATH = '192.168.1.101', 3000, '/upload-image'   # PC running Express
BAUD, UART_TX, UART_RX = 57600, 17, 16
W, H = 256, 288
PACKED_LEN = (W * H) // 2
SENSOR_ADDRESS = 0xFFFFFFFF

# Enrollment plan: two fingers × two images each
FINGERS = ["thumb", "index"]
...
def detect_mode(uart, f):
    ...
    print("DETECT mode: person_id={} — capture INDEX and THUMB, 2 images each.".format(person_id))
