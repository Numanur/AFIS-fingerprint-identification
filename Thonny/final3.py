# main.py — ESP32 + R307 detect/cls with post-capture memory cleanup.
...
HOST, PORT, PATH = '192.168.1.109', 3000, '/upload-image'   # PC running Express
...
# Optional: set a GC threshold so MicroPython collects proactively
try:
    gc.collect()
...
def http_post_start(host, port, path, body_len, headers_dict):
    ...
    lines = [
        ...
        "Connection: close",   # server will close after reply
    ]
...
def clear_after_cycle(uart, f, extra_objs=None):
    ...
    try:
        ...
        pass
    # Drop references to large temporaries if provided
    if extra_objs:
        for obj in extra_objs:
            try:
                ...
                pass
    # Double GC with a short delay can help on constrained heaps
    try:
        gc.collect()
        time.sleep_ms(10)
        gc.collect()
        ...
        pass
    # Make sure sensor is quiet and ready for next scan
    try:
        ensure_idle(f)
    except Exception:
        pass
...
def detect_mode(uart, f):
    ...
    while collected < SAMPLES_PER_PERSON:
        ...
        headers = {
            ...
            "X-Mode": "detect",
            "X-Filename": "{}_{}".format(person_id, collected + 1),
            "X-Identify": "0",
        }
        sock = None
        head_str = None
        ...
        try:
            ...
            print("Saved capture {}/{} ({} bytes). Server said: {}".format(
                collected+1, SAMPLES_PER_PERSON, sent, (body or b"").decode("utf-8","ignore")))
        ...
        wait_for_lift(f)
        # Aggressive cleanup
        clear_after_cycle(uart, f, extra_objs=[sock, head_str, body, sent])
    ...
    while True:
        ...
        try:
            ...
            try:
                ...
                # accept either match_id or matchId
                mid = res.get("match_id", None)
                if mid is None:
                    mid = res.get("matchId", None)
        ...
        wait_for_lift(f)
        # Aggressive cleanup
        clear_after_cycle(uart, f, extra_objs=[sock, head_str, body, text, res, sent])
